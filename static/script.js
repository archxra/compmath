...
function updateForm() {
    ...
    if (method === "1") { // Task 1
        formContainer.innerHTML = `
            <p>Graphical Method for f(x)=x⁴-10x²+9 over [-4,4].</p>
        ...
        formContainer.innerHTML = `
            ...
            <p>If fields are empty, defaults (initial guess=0, tol=1e-6, max_iter=100, ω=0.8) will be used.</p>
        ...
        formContainer.innerHTML = `
            <p>Enter data points for exponential curve fitting. If left empty, defaults are used:</p>
            <p>x: 0,1,2,3 &nbsp;&nbsp; y: 1,2.71828,7.38906,20.0855</p>
        ...
        formContainer.innerHTML = `
            <p>Enter data points for cubic spline interpolation. If left empty, defaults are used:</p>
            <p>x: 0,0.5,1.0,1.5 &nbsp;&nbsp; y: 0,0.25,0.75,2.25</p>
            <label for="x_values">x values (comma-separated):</label>
        ...
        formContainer.innerHTML = `
            <p>Solve dy/dx = x + y, y(0)=1 up to the 4th approximation.</p>
            <p>If left empty, x will default to 0.2 for evaluation.</p>
            <label for="picard_x">Evaluate y at x = </label>
            <input type="number" step="0.01" id="picard_x" placeholder="Default: 0.2">
        `;
    } else if (method === "8") { // Task 8: Simpson’s 1/3 Rule
        formContainer.innerHTML = `
            ...
            <input type="text" id="x_values_integral" placeholder="Default: auto-generated">
...
function compute() {
    ...
    if (method === "1") { // Task 1
        ...
        let x_values = document.getElementById("x_values").value;
        let y_values = document.getElementById("y_values").value;
        params = {
            x_values: x_values.trim() !== "" ? x_values : "0,1,2,3",
            y_values: y_values.trim() !== "" ? y_values : "1,2.71828,7.38906,20.0855"
        };
    } else if (method === "6") { // Task 6
        let x_values = document.getElementById("x_values").value;
        let y_values = document.getElementById("y_values").value;
        params = {
            x_values: x_values.trim() !== "" ? x_values : "0,0.5,1.0,1.5",
            y_values: y_values.trim() !== "" ? y_values : "0,0.25,0.75,2.25"
        };
        ...
        params = {
            ...
            n: parseInt(document.getElementById("n").value) || 11
        };
    }
...
function displayResult(data) {
    ...
    // Otherwise, if result is nested (for tasks 2-8)
    else if (data.result) {
        // If the result contains nested objects (like Task 2)
        if (data.result.hasOwnProperty("Bisection") || data.result.hasOwnProperty("Newton-Raphson")) {
            for (let m in data.result) {
                let subResult = data.result[m];
                outputHTML += `<h4>🔹 ${m}:</h4>`;
            ...
            for (let key in data.result) {
                ...
                if (typeof value === "object" && !Array.isArray(value)) {
                    // If it's a plain object, stringify it for display
                    outputHTML += `<p>🔹 <strong>${key}:</strong> ${JSON.stringify(value)}</p>`;
...
document.addEventListener("DOMContentLoaded", function() {
    updateForm(); // Ensure the dynamic form is populated on page load.
